    ...
    def __call__(self, sample, context=None):
        ...
        if isinstance(sample, Sequence):
            for i in range(len(sample)):
                sample[i] = self.apply(sample[i], context) #单个运行
        else:
            sample = self.apply(sample, context) #多个运行
        return sample
...
@register_op
class YoloxTransform(BaseOperator):
    def __init__(self):
        super(YoloxTransform, self).__init__()
    ...
    def apply_image(self, image, scale):
        print("apply image not implemented !!!")
        return None
    ...
    def __call__(self, sample, context=None):
        ...
        if isinstance(sample, Sequence):
            ...
            # 3 additional image indices 这一步已经在dataset那一步搞定, 会送4张图片进来
            for i_mosaic in range(len(sample) - 1):

                _labels = np.concatenate((sample[i_mosaic]["gt_bbox"], sample[i_mosaic]["gt_class"]), axis=1)
                im = sample[i_mosaic]['image']
                h0, w0, c0 = im.shape
                scale = min(1. * input_h / h0, 1. * input_w / w0)
                img = cv2.resize(
                    im, (int(w0 * scale), int(h0 * scale)), interpolation=cv2.INTER_LINEAR
                )
            ...
            if self.enable_mixup and not len(mosaic_labels) == 0:
                mosaic_img, mosaic_labels = self.mixup(mosaic_img, mosaic_labels, self.input_dim, sample[4])

            mix_img, padded_labels = yolox_preproc(mosaic_img, mosaic_labels, self.input_dim)
            # img_info = (mix_img.shape[1], mix_img.shape[0])

            # if padded_labels.nonzero()[0].shape[0] == 0 :
            #     print("---debug: ", mosaic_labels.nonzero()[0].shape, sample[0]["im_id"])
            #     print("bug")

            sample[0]["image"] = mix_img
            sample[0]["gt_bbox"] = padded_labels[:, 1:5]
            sample[0]["gt_class"] = np.expand_dims(padded_labels[:, 0], axis=1)

            sample[0]['im_shape'] = np.asarray([input_h, input_w], dtype=np.float32)
            return sample[0]

        else:
            img = sample['image']
            targets = np.concatenate((sample["gt_bbox"], sample["gt_class"]), axis=1)
            img, label = yolox_preproc(img, targets, self.input_dim)

            sample["image"] = img
            sample["gt_bbox"] = label[:4]
            sample["gt_class"] = label[4]
            sample['im_shape'] = np.asarray([input_h, input_w], dtype=np.float32)
            return sample

    def mixup(self, origin_img, origin_labels, input_dim, ms_sample):
        ...
        cp_labels = np.concatenate((ms_sample["gt_bbox"], ms_sample["gt_class"]), axis=1)
        # print("mixup: ", ms_sample["im_id"], cp_labels.shape, origin_labels.shape)
