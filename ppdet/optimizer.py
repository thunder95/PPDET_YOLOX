...
class YoloxWarmCosLr(LRScheduler):
    def __init__(self, learning_rate=0, min_lr=0, warmup_total_iters=1, warmup_lr_start=0,
                 total_iters=1, no_aug_iter=0, last_epoch=-1, verbose=False):
        ...
        super(YoloxWarmCosLr, self).__init__(learning_rate, last_epoch, verbose)

    # 注意 这里放在内循环, 记录的是iters
    def get_lr(self):
        if self.last_epoch <= self.warmup_total_iters:
            lr = (self.lrate - self.warmup_lr_start) * pow(
                self.last_epoch / float(self.warmup_total_iters), 2
            ) + self.warmup_lr_start

        elif self.last_epoch >= self.total_iters - self.no_aug_iter:
            lr = self.min_lr
        else:
            lr = self.min_lr + 0.5 * (self.lrate - self.min_lr) * (
                    1.0
                    + math.cos(
                        math.pi
                        * (self.last_epoch - self.warmup_total_iters)
                        / (self.total_iters - self.warmup_total_iters - self.no_aug_iter)
                    )
            )

        print("min lr: {}, cur lr: {}".format(self.min_lr, lr))
        return lr


@serializable
class YoloxWarmCosDecay(object):
    def __init__(self, max_epochs=1000, warmup_epochs=1, warmup_lr_start=0, no_aug_epochs=0, min_lr_ratio=0.2):
        self.max_epochs = max_epochs
    ...
    def __call__(self,
        ...
        assert base_lr is not None, "either base LR or values should be provided"
        assert step_per_epoch is not None, "step_per_epoch should be provided"

        total_iters = self.max_epochs * int(step_per_epoch)
        warmup_total_iters = step_per_epoch * self.warmup_epochs
        min_lr = base_lr * self.min_lr_ratio
        no_aug_iters = step_per_epoch * self.no_aug_epochs
        return YoloxWarmCosLr(base_lr, min_lr, warmup_total_iters, self.warmup_lr_start,
                              total_iters, no_aug_iters)
...
class LearningRate(object):
    ...
    def __call__(self, step_per_epoch):
        ...
        #yolox_warm_cos_lr
        if isinstance(self.schedulers[0], YoloxWarmCosDecay):
            return self.schedulers[0](self.base_lr, step_per_epoch)
